...
echo '<div style="font-size: 16px ;">';
    ...
    if(!empty($_POST)){
      $_SESSION["id_produit_select"] = $_POST["id_produit"];//stocké en session pour pouvoir la réutiliser facilement en cas d'erreur avec les infos données
    }

    //echo "produit : ".$id_produit;

    //$idItemSelect=$id_produit; //id du produit que le souhaite payer
    $idItemSelect=$_SESSION["id_produit_select"]; //id du produit que le souhaite payer
    ...
    $itemPriceEuro=$itemPrice/100;
    $pt_fidelite=calculFidelite($itemPriceEuro);
    $max_reduc_euro=$itemPriceEuro*0.9; // Reducion avec point fidélité = -90%
...
<?php
    echo $user_firstname.' '.$user_lastname.$user_fidelity;
    echo "<br> Tu gagneras ";
    echo $pt_fidelite;
    ...
    <h3><?php echo "Article : ".$itemName." | Prix : ".$produit['prix_produit']."€"."<br>" ; ?></h3>
    ...
    <form action="verify_fidelity_point_shop.php" method="post" id="payment-form">
      <div>
      ...
      <div id="card-element" style="background-color: #FFF; width: 250px; position: relative; left: 150px;">
        ...
        <input type="hidden" value="<?php echo $itemID;?>" name="id_produit">
